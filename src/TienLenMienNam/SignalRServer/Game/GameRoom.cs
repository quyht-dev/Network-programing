...
namespace CardGameServer.Game
{
    ...
    public sealed class GameRoom
    {
        ...
        // Tối đa 4 người chơi
        private readonly List<IPlayerSession> _players = new List<IPlayerSession>();
        ...
        public bool Contains(IPlayerSession s)
        {
            lock (_gate) return _players.Contains(s);
        }

        public bool Join(IPlayerSession s, string playerName, out string error)
        {
            error = null;
            lock (_gate)
            {
        ...
        public void Leave(IPlayerSession s)
        {
            lock (_gate)
            {
                _players.Remove(s);
        ...
        public bool SetReady(IPlayerSession s, bool ready, out string error)
        {
            error = null;
            lock (_gate)
            {
        ...
        public bool Pass(IPlayerSession s, out string error)
        {
            error = null;
